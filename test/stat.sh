#!/bin/bash
 ...
 echo "  - iostat.v2 second request, measured 5 seconds -"
...
util_output=$(iostat -x 5 2 | awk '/^Device/{count++; if (count == 2) {found=1; next}} found && /^nvme/ {print $1, $23}') # $23 соответствует %util
